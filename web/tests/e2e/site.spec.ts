  ...
  for (const id of TAB_IDS) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/^https?:\/\//.test(href)) {
        ...
        // /moment/api/. It is produced by the Go doc generator at deploy time,
...
test('docs tab renders the React API reference with a package listed', async ({ page }) => {
  ...
  // moment is a single-package module (package moment), so at least one package link is expected.
